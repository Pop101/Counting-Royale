...
class Database():
    ...
    def create_table(self, name:str, columns:dict, hidden_columns:dict or None={'last_modified': float, 'times_modified': int}, key_index:int = 0):
        ...
        def convert(pytype:type) -> str:
            ...
            elif pytype == bool: return 'boolean'
...
class Table():
    ...
    def fetch_columns(self, show_hidden_columns:bool = False, scan_for_dbobjects:bool = True) -> list:
        ...
        def convert(sqltype:str, colname:str=None) -> type:
            ...
            elif 'boolean' in sqltype: return bool
    ...
    def fetch_entries(self, conditions:list or str = None, show_hidden_columns:bool = False, limit:int = -1, offset:int = 0) -> list:
        ...
        subscriptable_headers = list(self.raw_columns.items())
        rows = list(rows)
        for i in range(len(rows)):
            rows[i] = list(rows[i])
            for j in range(len(rows[i])):
                if subscriptable_headers[j][1] == bool: 
                    rows[i][j] = bool(rows[i][j]) # sql stores bools as 0 and 1, so use table header to remember and cast
        ...
        for k, v in self.raw_columns.items():
            ...
            if k not in values:
                ...
                # If ommitted and shouldn't fill
                    ...
                    raise ValueError(f'Values given do not match columns: {self.columns}. Check {k}')
    ...
    def remove_entries(self, conditions:str or list, limit:int or str = 'row_count') -> bool:
        # Vulnerable to SQL injects
        if isinstance(conditions, list) or isinstance(conditions, tuple):
            conditions = ' AND '.join(conditions)
        
        self.database.execute(f'''DELETE * FROM {self.name} WHERE {conditions} LIMIT {limit}''')
        return True
...
class Configuration(Table):
    ...
    def set(self, key:Any or int, setting:str, value:Any):
        ...
        try: has_set = len(self.database.execute(f'''SELECT {Configuration.KEY_COLUMN_NAME} FROM {self.name} WHERE {Configuration.KEY_COLUMN_NAME}={key} LIMIT 1''')) > 0
    ...
    def set_all(self, key:Any or int, settings:dict):
        ...
        if has_set:
            ...
            self.add_entry({**self.defaults, **settings, **{Configuration.KEY_COLUMN_NAME: key}})
